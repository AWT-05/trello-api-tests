...
Feature: Test POST, GET, PUT and DELETE without authorization
  ...
  Background: Set initial state and authentication
    ...
    * I have a label created
    And I set authentication using only an API key


  Scenario: Try to create a label with only an API key
  ...
  Scenario: Try to get a label with only an API key
  ...
  Scenario: Try to update a label with only an API key
  ...
  Scenario: Try to delete a label with only an API key
